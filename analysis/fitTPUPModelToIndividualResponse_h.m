function [theResult] = fitTPUPModelToIndividualResponse(accumStimTypesResponse,accumStimTypesStimulus, avgPackets)
...
fprintf('>> Fitting individual amplitude model to data (TPUP)\n');
...
for ss = 1:size(accumStimTypesRespRaw,1) % Looping over subjects
    ...
    for cc = 1:(size(accumStimTypesRespRaw,2)-1) % Looping over stimulus types, skipping attention tasks
        ...
        for ii = 1:size(accumStimTypesRespRaw{ss,cc},1)
            ...
            % that.
            TPUPFitToData{ss,cc}{1,ii}.paramsFit=paramsFit;
            TPUPFitToData{ss,cc}{1,ii}.fVal=fVal;
            TPUPFitToData{ss,cc}{1,ii}.modelResponseStruct=modelResponseStruct;
